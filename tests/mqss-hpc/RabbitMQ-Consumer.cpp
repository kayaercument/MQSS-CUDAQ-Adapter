...
amqp_connection_state_t setupConnection(const std::string &queue_name) {
  ...
  // Declare the queue
  amqp_queue_declare(conn, 1, amqp_cstring_bytes(queue_name.c_str()), 0, 1, 0,
                     0, amqp_empty_table);
...
void answerRequestToQueue(const amqp_connection_state_t &conn,
                          ...
                          const std::string &correlation_id, bool isJSON) {
...
void processConnection(std::function<nlohmann::json(std::string)> function,
  ...
  while (true) {
    ...
    readRequestFromQueue(conn, queue_name, envelope, message, reply_to,
                         correlation_id);
#ifdef DEBUG
...
int main() {
  ...
  threads.push_back(
      std::thread(processConnection, getJobStatus, queueJobString));
