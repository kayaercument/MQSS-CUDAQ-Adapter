...
------------------------------------------------------------------------------
  ...
  When processing the job, the consumer lower the quake code to QIR and the
...
void getJobStatus(const std::string &jobId, const std::string &replyQueue,
  ...
  // Simulate asynchronous behavior by returning "running" for the first few
  // requests
  ...
  replyServer.publishMessage(replyQueue, resultResponse.dump(), correlationId,
                             true);
}

void processJob(const std::string &receivedMessage,
                const std::string &replyQueue, const std::string &correlationId,
                std::string taskId) {
  // return job status to the rabbitmq client using the replyqueue
  RabbitMQServer replyServer(AMQP_SERVER, AMQP_PORT, QUEUE_HPC_OFFLOADER,
                             AMQP_USER, AMQP_PASSWORD);
  ...
  if (!jobData.contains("name") || !jobData.contains("count") ||
      !jobData.contains("program")) {
    replyServer.publishMessage(replyQueue, errorResponse.dump(), correlationId,
                               true);
    return;
  }
  ...
  std::unordered_map<std::string, std::unordered_map<int, int>> results =
      parseStringToMap(resultCircuit);
...
int main() {
  ...
  while (true) {
    ...
    try {
      ...
      threadsConnections.push_back(std::thread(processJob, message, replyQueue,
      ...
      threadsConnections.push_back(
          std::thread(getJobStatus, message, replyQueue, correlationId));
      std::cout << "Checking status of task with id: {}" << message
                << std::endl;
    }
  }
