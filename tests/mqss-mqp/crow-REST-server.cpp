  ...
  @ brief
        ...
        The HTTP server responds to the following queries:
                ...
                - "/job" this creates a job in the server, as described in the
                  ...
                  name, count (shots) and the program (quake). It returns a job
 ID used latter to veriy the job status or to obtain the results.
...
crow::response getJobStatus(const std::string &jobId) {
  ...
  // requests Check if the job exists
...
void startServer(int port) {
  ...
  CROW_ROUTE(app, "/job")
      .methods(crow::HTTPMethod::POST)([](const crow::request &req) {
...
#ifdef DEBUG
        std::cout << "Json jobData: " << jobData << std::endl;
#endif
        if (!jobData || !jobData.has("name") || !jobData.has("n_shots") ||
            !jobData.has("circuit_files")) {
        ...
        std::string command =
            std::string("python3 ./SimulateKernelLoweredToLLVM.py -c "
                        "tempCircuit.txt -o tempResults.txt -s 1000");
        // Execute the command
        ...
        // Simulate results (in the original, this comes from some quantum
        // function)
