...
protected:
  ...
  std::string token =
      "6XHDN1U8WIxourN4lGP8OEuu3zDRI5Y1LoxJN6dAT7iftnpBlZ8tZ9pybZ05ipkh";
...
MQSSServerHelper::createJob(std::vector<KernelExecution> &circuitCodes) {
  ...
  for (auto &circuitCode : circuitCodes) {
    ...
    timeStream << std::put_time(std::localtime(&currentTime),
                                "%Y-%m-%d %H:%M:%S");
...
std::string MQSSServerHelper::constructGetJobPath(ServerMessage &postResponse) {
...
MQSSServerHelper::processResults(ServerMessage &postJobResponse,
                                 std::string &jobId) {
  // MQP has a separated request for asking results, this only will work if it
  // is fired after we are 100% sure, the job is completed
  RestClient client;
  auto headers = generateRequestHeader();
  auto resultResponse =
      client.get(mqpUrl + "job/" + jobId + "/result", "", headers);
...
RestHeaders MQSSServerHelper::getHeaders() { return generateRequestHeader(); }
...
std::string searchMQSSAPIKey(std::string &key, std::string &refreshKey,
  ...
  if (cudaq::fileExists(hwConfig)) {
    ...
    throw std::runtime_error("Cannot find MQSS Config file with credentials "
                             "(~/.mqssMQP_config).");
  }
  return hwConfig;
}
...
CUDAQ_REGISTER_TYPE(cudaq::ServerHelper, cudaq::MQSSServerHelper, mqssMQP)
